                            ...
                            <div class="col-lg-5">
                                <div class="card shadow-lg border-0 rounded-lg mt-5">
                                    ...
                                    <div class="card-footer text-center">
                                        <div class="small"><a href="<?=base_url()?>AdminController/register">Need an account? Sign up!</a></div>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            $(document).ready(function(){
                $("#loginBtn").on('click', function(e){
                    ...
                    if(emailError == "" && pswdError == ""){
                        $.ajax({
                            url: "<?=base_url()?>AdminController/checklogin",
                            type: "POST",
                            data: {email: user_email, password: user_pswd},
                            success: function(response){
								console.log(response);
                                if(response == "Yes"){
                                    window.location.href ="<?=base_url()?>AdminController/managecategory";
                                }else{
